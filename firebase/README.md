...
- Firebase Authentication: email/password; existing Supabase UIDs and bcrypt hashes imported.
...
Sign in with the existing email/password after upgrading. Old Supabase session tokens are not Firebase sessions. The installation is bound to its first Firebase UID to prevent uploading one person's local library to another account. To change accounts, export local data and clear the installation first.
...
Imported and verified 4 auth accounts, 21 native key/value modules, 4 profiles, 4 shared spaces, 4 memberships, and 1 shared item. Three older web-format records are preserved losslessly under each user's `legacyRecords` collection; their UUID schemas do not match the native Long-ID schema. Source attachment storage, Telegram links, and FCM registrations were empty. The old Supabase project is retained for recovery; the updated app makes no Supabase calls.
