...
The client exchanges local text and its last acknowledged server baseline. The server merges objects and record arrays by stable `id` inside a transaction. Independent fields and different records survive concurrent edits; when the same field is changed on both devices, the incoming edit wins. New native records use OS-backed random 53-bit IDs, independently generated on each device and exactly representable by the Node gateway. Existing IDs and references remain unchanged. This removes the shared-counter collision problem; uniqueness is probabilistic, not a server-issued guarantee. Device-local alarm IDs are allocated separately, and record IDs no longer imply creation order. All devices should upgrade before creating records offline. Non-record arrays are merged as whole values. Local edits made during a request are merged back before acknowledgement. Local file writes use a synced temporary file and rename.
