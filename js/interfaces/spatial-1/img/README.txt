...
material for each of the 8 signs (7 district doors + Station News) -- plus
...
built):
  ...
  angle. Render each of the 8 signs with its real final text fully
  ...
  exactly at that corner -- 32 markers total (8 signs x 4 corners). Keep
...
1. Detect markers: threshold the image for near-pure red pixels
   ...
   reject noise. Expect exactly 32 candidates (8 signs x 4 corners) --
...
## Retired: immersive district rooms (2026-07-13)

The Conservatory district (the only district with an immersive `room` --
a full establishing-shot image with perspective-projected signage) was
removed when Languages and Chords were cut from the app. The room-signage
pipeline that used to live in this section (marker-based text extraction,
recoloring, and later a measured-homography CSS projection) is gone along
with it -- conservatory.png, consred.png, consredout.png, credot*.png were
all specific to that one room and have been deleted. If a future district
gets its own immersive room again, that technique would need to be
rebuilt from scratch; nothing here documents a pattern worth preserving
independent of the specific room it was built for.
